...
app.get("/authed", async (req, res) => {
 ...
 //  commandLogsChannel.send({
   ...
   // })
   // client.destroy();
   // setTimeout(someFunc, 75000);
   // client.login(Token);
     // commandLogsChannel.send({
    
     // embeds: [{
       //title: `Auto restart`,
      // description: `Restart went successfully.`,
...
const db = require('quick.db');
    if (!Owners.includes(interaction.user.id) &&  db.get(`wl_${interaction.user.id}`) !== true ) return interaction.reply(`You may not use this.`)
...
// https://discord.com/api/webhooks/1055604777469358112/g897NlflLbqFjkE98WyOyapN46Zb-B0u9WhqNO3b-vNI63N7mliWK0LmIMvrrhwL2gPz